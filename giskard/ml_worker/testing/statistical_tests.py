...
class StatisticalTests(AbstractTestCollection):
    ...
    def test_disparate_impact(self,
        ...
        protected_predictions = np.squeeze(model.run_predict(protected_ds_po).raw_prediction == positive_idx)
        unprotected_predictions = np.squeeze(model.run_predict(unprotected_ds_po).raw_prediction == positive_idx)
        ...
        return self.save_results(
            SingleTestResult(
                ...
                passed=disparate_impact_score > threshold
            )
        )